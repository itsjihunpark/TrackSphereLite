...
class MotionFiler:
    ...
    def __init__(self, frame_producer, roi):
        ...
        if motion or self.release_frames:
            ...
            if self.release_frames:
                if self.release_counter == 0:
                    self.release_frame = False
                    self.release_counter = self.release_n_frames
                self.release_counter -= 1
            filter_flag = True

        print(filter_flag, total_contour_area, motion, self.release_frames, self.release_counter)
